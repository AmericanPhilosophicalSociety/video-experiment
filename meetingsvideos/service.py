...
def basic_search(query):
    ...
    video_search = (
        ...
        | Q(title__search=query)
        | Q(abstract__search=query)
        | Q(speakers__display_name__search=query)
        | Q(speakers__lcsh__heading__search=query)
        | Q(lcsh__heading__search=query)
    )
    ...
    video_icontains = (
        ...
        | Q(speakers__display_name__icontains=query)
        | Q(speakers__lcsh__heading__icontains=query)
        | Q(lcsh__heading__icontains=query)
    )
    ...
    speaker_search = (
        Q(display_name__search=query)
        | Q(lcsh__heading__search=query)
        | Q(affiliation__position__search=query)
        | Q(affiliation__institution__search=query)
    )

    speaker_icontains = (
        Q(display_name__icontains=query)
        | Q(lcsh__heading__icontains=query)
        | Q(affiliation__position__icontains=query)
        | Q(affiliation__institution__icontains=query)
    )
    ...
    subjects = LCSH.objects.filter(
        Q(heading__search=query) | Q(heading__icontains=query)
    ).exclude(video=None)
